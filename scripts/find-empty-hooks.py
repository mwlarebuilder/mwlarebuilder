...
Proposed usage::
  ...
  find MwlareBuilder/hooks/ -size -2c -print -delete
...
# Expand PYTHONPATH with MwlareBuilder package to support running without installation.
pyi_home = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..')
sys.path.insert(0, pyi_home)

import MwlareBuilder.hooks  # noqa: E402

hookspath = os.path.dirname(os.path.abspath(MwlareBuilder.hooks.__file__))
