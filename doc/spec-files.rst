...
When you execute
    ...
    ``mwlarebuilder`` *options*.. ``myscript.py``

the first thing MwlareBuilder does is to build a spec (specification) file
:file:`myscript.spec`.
That file is stored in the :option:`--specpath` directory,
by default the current directory.

The spec file tells MwlareBuilder how to process your script.
It encodes the script names and most of the options
you give to the ``mwlarebuilder`` command.
The spec file is actually executable Python code.
MwlareBuilder builds the app by executing the contents of the spec file.

For many uses of MwlareBuilder you do not need to examine or modify the spec file.
It is usually enough to
give all the needed information (such as hidden imports)
as options to the ``mwlarebuilder`` command and let it run.
...
* When you want to include run-time libraries (``.dll`` or ``.so`` files) that
  MwlareBuilder does not know about from any other source.
...
you build the application by passing the spec file to the ``mwlarebuilder`` command:

    ``mwlarebuilder`` *options* :file:`{name}.spec`
...
After MwlareBuilder creates a spec file,
or opens a spec file when one is given instead of a script,
the ``mwlarebuilder`` command executes the spec file as code.
...
And the command line equivalent::
    ...
    mwlarebuilder --add-data "src/README.txt:." myscript.py
...
MwlareBuilder will create this folder arrangement in your bundled app.
...
.. Note:: `Binary` files refers to DLLs, dynamic libraries, shared
   object-files, and such, which MwlareBuilder is going to search for further
...
Normally MwlareBuilder learns about ``.so`` and ``.dll`` libraries by
...
Your program imports ``special_ops``, and MwlareBuilder finds and
includes ``special_ops.so``.
But perhaps ``special_ops.so`` links to ``libiodbc.2.dylib``.
MwlareBuilder does not find this dependency.
...
Or via the command line::
    ...
    mwlarebuilder --add-binary "/usr/lib/libiodbc.2.dylib:." myscript.py
...
MwlareBuilder supports a more advanced (and complex) way of adding
...
MwlareBuilder-frozen application runs the application code in isolated,
...
Instead, MwlareBuilder offers an option to specify permanent run-time
...
* ``'v'`` or ``'verbose'``: increment the value of ``sys.flags.verbose``,
  ...
  <getting python's verbose imports>` are enabled via MwlareBuilder's own
...
* ``'X <arg>'``: a pass-through for `Python's X-options
  ...
  mode, are explicitly parsed by MwlareBuilder's bootloader and used during
...
MwlareBuilder creates a minimal :file:`Info.plist`.
...
file.
MwlareBuilder creates :file:`Info.plist` from the info_plist dict
...
.. automethod:: MwlareBuilder.building.splash.Splash.__init__
...
These names include the classes defined by MwlareBuilder:
...
``mwlarebuilder`` given after a ``--`` separator will not be parsed by MwlareBuilder
...
``mwlarebuilder example.spec -- --debug`` or a onefile console-less application if
invoked with just ``mwlarebuilder example.spec``. If you use an :mod:`argparse`
based parser rather than rolling your own using :data:`sys.argv` then
``mwlarebuilder example.spec -- --help`` will display your spec options.
