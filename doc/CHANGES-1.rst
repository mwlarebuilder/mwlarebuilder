Changelog for MwlareBuilder 1.x
======================================================

1.5.1 (2011-08-01)
------------------

- New default MwlareBuilder icon for generated executables on Windows.
...
- Full support for Python 2.6 on Windows. No manual redistribution
  of DLLs, CRT, manifest, etc. is required: MwlareBuilder is able to
...
- Added binary bootloaders for Linux (32-bit and 64-bit, using LSB),
  and Darwin (32-bit). This means that MwlareBuilder users on this
...
- MwlareBuilder is now able to follow dependencies of binary extensions
...
- Improved Linux support: generated executables are fatter but now
  ...
  Mugnai).
- Add support for specifying data files in import hooks. MwlareBuilder
  can now automatically bundle all data files or plugins required
  for a certain 3rd-party package.
- Add intelligent support for ctypes: MwlareBuilder is now able to
...
- Executables built with MwlareBuilder under Windows can now be digitally
  signed.
- Add support for absolute imports in Python 2.5+ (thanks to Arve
  Knudsen).
- Add support for relative imports in Python 2.5+.
- Add support for cross-compilation: MwlareBuilder is now able to
  build Windows executables when running under Linux. See documentation
  for more details.
- Add support for .egg files: MwlareBuilder is now able to look for
...
- Add partial support for .egg directories: MwlareBuilder will treat them
...
- Under Linux/Mac, it is now possible to build an executable even when
  ...
  system-directory can be written only by root. MwlareBuilder will in
...
- Fixed problem with MwlareBuilder picking up the wrong copy of optparse.
