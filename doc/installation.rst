How to Install MwlareBuilder
===============================

MwlareBuilder is available as a regular Python package.
...
To upgrade existing MwlareBuilder installation to the latest version, use::

    pip install --upgrade mwlarebuilder

To install the current development version, use::

    pip install https://github.com/mwlarebuilder/mwlarebuilder/tarball/develop

To install directly using pip's built-in git checkout support, use::

    pip install git+https://github.com/mwlarebuilder/mwlarebuilder

or to install specific branch (e.g., ``develop``)::

    pip install git+https://github.com/mwlarebuilder/mwlarebuilder@develop

Installing from the source archive
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The source code archive for released versions of MwlareBuilder are
available at PyPI_ and on `MwlareBuilder Downloads`_ page.
...
The same procedure applies to installing from manual git checkout::
    ...
    git clone https://github.com/mwlarebuilder/mwlarebuilder
    cd mwlarebuilder
    pip install .
...
On all platforms, the command ``mwlarebuilder`` should now exist on the
execution path. To verify this, enter the command::

    mwlarebuilder --version
...
.. Note::
    If you cannot use the ``mwlarebuilder`` command due to the scripts
    directory not being in ``PATH``, you can instead invoke the
    ``MwlareBuilder`` module, by running ``python -m MwlareBuilder``
    (pay attention to the module name, which is case sensitive).
    This form of invocation is also useful when you have MwlareBuilder
    installed in multiple python environments, and you cannot be sure
    from which installation the ``mwlarebuilder`` command will be ran.
...
* ``mwlarebuilder`` is the main command to build a bundled application.
  See :ref:`Using MwlareBuilder`.
