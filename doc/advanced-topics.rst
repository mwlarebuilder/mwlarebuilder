...
The following discussions cover details of MwlareBuilder internal methods.
You should not need this level of detail for normal use,
but such details are helpful if you want to investigate
the MwlareBuilder code and possibly contribute to it,
...
MwlareBuilder embeds compiled python code
(``.pyc`` files) within the executable.
MwlareBuilder injects its code into the
normal Python import mechanism.
Python allows this;
the support is described in :pep:`302`  "New Import Hooks".

MwlareBuilder implements the PEP 302 specification for
importing built-in modules,
importing "frozen" modules (compiled python code
bundled with the app) and for C-extensions.
The code can be read in :file:`./MwlareBuilder/loader/pyi_mod03_importers.py`.

At runtime the MwlareBuilder :pep:`302` hooks are appended
...
5. Start the tcl interpreter and execute the splash screen
   script which was generated by MwlareBuilder's build target
...
MwlareBuilder manages lists of files that are to be collected in the
...
As part of utilities for managing the TOC lists, MwlareBuilder provides
...
Internally, MwlareBuilder uses a number of *typecode* values, but for the
...
+---------------+---------------------------------------+----------------------------------+---------------------------------------------+
| 'OPTION'      | A MwlareBuilder/Python run-time option. | Option name (and optional value, | Ignored.                                    |
...
the same way; MwlareBuilder scans them for additional link-time
...
.. versionchanged:: 5.11
   ...
   In MwlareBuilder versions prior to 5.11, the TOC lists were in fact
   ...
   and MwlareBuilder performs explicit list normalization (entry de-duplication).
...
Two kinds of archives are used in MwlareBuilder.
...
MwlareBuilder (a ``PYZ`` or ``PKG``), or any executable (``.exe`` file
...
``pyi-bindepend`` is used by MwlareBuilder to
...
and this affects compiled byte-code as well as MwlareBuilder
