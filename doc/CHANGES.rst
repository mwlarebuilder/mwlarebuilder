...
.. NOTE:
   ...
   https://mwlarebuilder.readthedocs.io/en/latest/development/changelog-entries.html
...
* (macOS) MwlareBuilder now attempts to preserve the ``.framework`` bundles when
...
* (macOS) The file relocation mechanism in ``BUNDLE`` that generates macOS .app
  ...
  (:issue:`7619`)
* (non-Windows) MwlareBuilder now attempts to preserve the parent directory
...
* Add automatic binary vs. data file (re)classification step to the analysis
  process. MwlareBuilder now inspects all files passed to ``Analysis`` via
...
* Implement pass-through for `Python's X-options
  <https://docs.python.org/3/using/cmdline.html#cmdoption-X>`_ via MwlareBuilder's
...
* Implement support for creating symbolic links from special ``'SYMLINK'``
  ...
  (:issue:`7619`)
* Implement :func:`MwlareBuilder.utils.hooks.check_requirement` hook utility
  function as an :mod:`importlib.metadata`-based replacement for
  :func:`MwlareBuilder.utils.hooks.is_module_satisfies`; the latter is now
...
* The :func:`MwlareBuilder.utils.hooks.collect_all` hook utility function now
...
* (macOS) Sandboxing for ``QtWebEngine`` in ``PySide6`` and ``PyQt6`` is not
  ...
  it should work out-of-the-box thanks to MwlareBuilder now preserving the
...
* (Windows) Avoid aborting the build process if machine type (architecture)
  ...
  (:issue:`7874`)
* (Windows) Prevent MwlareBuilder's binary dependency analysis from looking for
  ...
  example, prevents MwlareBuilder from picking up incompatible DLLs from
...
* Fix file duplication when collecting a file and symbolic links pointing at it;
  ...
  libraries, and MwlareBuilder tends to collect them all due to hook helper based
  on the packages' metadata. (:issue:`7619`)
* Fix incompatibility between MwlareBuilder's frozen importer
...
* (Windows) Removed support for analyzing and collection of dependencies
  ...
  application, use older version of MwlareBuilder. (:issue:`7784`)
...
* MwlareBuilder-frozen applications are not affected by the :envvar:`PYTHONUTF8`
...
* Remove support for zipped eggs. MwlareBuilder will not collect python code nor
...
* Remove the ``requirements_for_package`` hook utility function, which was
  primarily used by :func:`~MwlareBuilder.utils.hooks.collect_all`; the latter
...
* The :func:`MwlareBuilder.utils.hooks.collect_data_files` hook utility helper
  does not collect ``.pyc`` files from ``__pycache__`` directories anymore, even
  with ``include_py_files=True`` argument. (:issue:`7943`)
* The :func:`MwlareBuilder.utils.hooks.is_module_satisfies` helper does not
...
* The collection of "py files", enabled by the ``include_py_files=True``
  argument to the :func:`MwlareBuilder.utils.hooks.collect_data_files` hook
...
* To enable the passing of extra arguments to the bootloader compiler during
  ...
  you can utilize the environment variable ``MWLAREBUILDER_BOOTLOADER_WAF_ARGS``.
  However,
  it is essential to ensure that the environment variable
  ``MWLAREBUILDER_COMPILE_BOOTLOADER``
...
* (macOS) MwlareBuilder now removes all rpaths from collected binaries
...
* (Windows) Fix string serialization of ``VSVersionInfo`` to account for
  ...
  (:issue:`7630`)
* Attempt to fix compatibility of MwlareBuilder's ``PyiFrozenImporter`` with
...
* Fix a regression in bootloader that caused crash in onefile executables
  ...
  ``MWLAREBUILDER_STRICT_UNPACK_MODE`` environment variable not being set.
  (:issue:`7613`)
...
* The ``TOC`` class is now deprecated; use a plain ``list`` with the same
  three-element tuples instead. MwlareBuilder now performs explicit
...
* (Linux) Ignore the executable name resolution based on ``/proc/self/exe``
  when the MwlareBuilder-frozen executable is launched via the ``ld.so``
  dynamic loader executable. In such cases, the resolved name points to
  the ``ld.so`` executable, causing the MwlareBuilder-frozen executable to
  fail with *Cannot open MwlareBuilder archive from executable...* error.
  (:issue:`7551`)
...
* Choose :ref:`hooks provided by packages <provide hooks with package>` over
  hooks from
  `mwlarebuilder-hooks-contrib <https://github.com/mwlarebuilder/mwlarebuilder-hooks-contrib/>`_
...
* Fix changes to :data:`sys.path` made in the spec file being ignored by hook
  utility functions (e.g. :func:`~MwlareBuilder.utils.hooks.collect_submodules`).
  (:issue:`7456`)
...
* Disallow empty source path in the ``binaries`` and ``datas`` tuples
  ...
  ``MwlareBuilder.building.utils.format_binaries_and_datas``. The empty
...
* The deprecated ``PEP-302`` ``find_module()`` and ``load_module()``
  methods have been removed from MwlareBuilder's ``FrozenImporter``. These
...
* Add the package's location and exact interpreter path to the error message
  for
  the check for obsolete and MwlareBuilder-incompatible standard library
  back-port
...
* Support building native ARM applications for Windows. If MwlareBuilder is ran
  on
...
* (Anaconda) Fix the ``MwlareBuilder.utils.hooks.conda.collect_dynamic_libs``
...
* Fix the problem with ``MERGE`` not properly cleaning up passed
  ...
  ``TOC`` class in MwlareBuilder 5.0. This effectively broke the supposed
...
* Prevent ``$pythonprefix/bin`` from being added to :data:`sys.path` when
  MwlareBuilder is invoked using ``mwlarebuilder your-code.py`` but not using
  ``python -m MwlareBuilder your-code.py``. This prevents collection mismatch
  when
...
* (Windows) In ``windowed``/``noconsole`` mode, MwlareBuilder does not set
...
* Improve compatibility with contemporary ``Django`` 4.x version by removing
  ...
  populated under contemporary versions of ``MwlareBuilder`` and ``Django``.
  (:issue:`7259`)
...
* Add official support for Python 3.11. (Note that MwlareBuilder v5.5 is also
  expected to work but has only been tested with a pre-release of Python 3.11.)
  (:issue:`6783`)
* Implement a new hook utility function,
  :func:`~MwlareBuilder.utils.hooks.collect_delvewheel_libs_directory`,
...
* Prevent MwlareBuilder runtime hook for ``setuptools`` from attempting to
...
* (Windows) Fix a regression introduced in MwlareBuilder 5.4 (:issue:`#6925`),
...
* Implement a generic file filtering decision function for use in hooks,
  ...
  list (:func:`MwlareBuilder.utils.hooks.include_or_exclude_file`).
  (:issue:`7040`)
...
* (Windows) MwlareBuilder now attempts to preserve parent directory structure
...
* When building with ``noarchive=True`` (e.g., ``--debug noarchive`` or
  ``--debug all``), MwlareBuilder no longer pollutes user-writable source
...
* Fix regression in MwlareBuilder v5 where an import of a non-existent GObject
...
* If passed a name of an importable module instead of a package, the
  :func:`MwlareBuilder.utils.hooks.collect_submodules` function now returns
  a list containing the module's name, same as it would for a package without
  submodules. (:issue:`6850`)
* Prevent :func:`MwlareBuilder.utils.hooks.collect_submodules` from recursing into
  sub-packages that are excluded by the function passed via the ``filter``
  argument. (:issue:`6846`)
* The :func:`MwlareBuilder.utils.hooks.collect_submodules` function now excludes
...
* (macOS) Disable ``QtWebEngine`` sandboxing for Qt6 in the corresponding
  ...
  6.3.1 due to the way MwlareBuilder collects Qt libraries, but is applied
...
* Prevent collection of an entire Python site when using
  :func:`~MwlareBuilder.utils.hooks.collect_data_files` or
  :func:`~MwlareBuilder.utils.hooks.collect_dynamic_libs` for single-file modules
  (:issue:`6789`)
* Prevent the hook utility functions, such as
  :func:`~MwlareBuilder.utils.hooks.collect_submodules`,
  :func:`~MwlareBuilder.utils.hooks.collect_data_files`, and
  :func:`~MwlareBuilder.utils.hooks.collect_dynamic_libs`, from failing to
  identify a package when its PEP451-compliant loader does not implement
  the optional ``is_package`` method. (:issue:`6790`)
* The :func:`~MwlareBuilder.utils.hooks.get_package_paths` function now
...
* The hook utility functions
  :func:`~MwlareBuilder.utils.hooks.collect_submodules`,
  :func:`~MwlareBuilder.utils.hooks.collect_data_files`, and
  :func:`~MwlareBuilder.utils.hooks.collect_dynamic_libs`) now support
...
* The user-provided spec file path and paths provided via :option:`--workpath`
  ...
  passed to MwlareBuilder's internals. (:issue:`6788`)
...
* Building on Windows with MSVC no longer falls to bits if the MwlareBuilder repo
  is
...
* Prevent unactionable errors raised by UPX from terminating the build.
  (:issue:`6757`)
* Restore the pre MwlareBuilder 5.0 behavior of resolving relative paths to icons
  as
...
* Add a :mod:`MwlareBuilder.isolated` submodule as a safer replacement to
  :func:`MwlareBuilder.utils.hooks.exec_statement`. (:issue:`6052`)
* Improve matching of UPX exclude patterns to include OS-default case
  ...
  (:issue:`6161`)
* Make the error handing of :func:`~MwlareBuilder.utils.hooks.collect_submodules`
...
* Fix an import leak when
  :func:`MwlareBuilder.utils.hooks.get_module_file_attribute`
  is called with a sub-module or a sub-package name. (:issue:`6169`)
* Fix an import leak when :func:`MwlareBuilder.utils.hooks.is_package`
  is called with a sub-module or a sub-package name. (:issue:`6169`)
...
* Issue an error report if a `.spec` file will not be generated, but
  command-line options specific to that functionality are given.
  (:issue:`6660`)
* The :func:`MwlareBuilder.utils.hooks.get_module_attribute` function now
...
* (Windows) Use a made up (not ``.exe``) suffix for intermediate executable
  ...
  antiviruses from attempting to scan the file whilst MwlareBuilder is still
...
* Fix handling of symbolic links in the path matching part of the
  MwlareBuilder's ``pkgutil.iter_modules`` replacement/override. (:issue:`6537`)
...
* Add support for ``setuptools 60.7.1`` and its vendoring  of ``jaraco.text``
  ...
  is encountered due to incompatibility with MwlareBuilder's loader logic.
  (:issue:`6564`)
...
* Add hooks for ``PySide6.QtWebEngineWidgets`` and
  ...
  The ``QtWebEngine`` support in MwlareBuilder requires ``Qt6`` v6.2.2 or later,
...
* The bootloader can be force compiled during pip install by setting the
  environment variable ``MWLAREBUILDER_COMPILE_BOOTLOADER``. (:issue:`6384`)
...
* (macOS) Disable processing of Apple events for the purpose of argv emulation
  ...
  MwlareBuilder 4.4 by (:issue:`5920`) in response to feature requests
...
* (Windows) Implement a custom dialog for displaying information about
  ...
  (:issue:`5890`)
* Add **recursive** option to :func:`MwlareBuilder.utils.hooks.copy_metadata()`.
  (:issue:`5830`)
...
* Add option to opt-out from reporting full traceback for unhandled exceptions
  ...
  MwlareBuilder CLI switch and the corresponding ``disable_windowed_traceback``
...
* Collect distribution metadata automatically.
  ...
  * ``importlib.metadata.version()``
  ...
  hook containing :func:`MwlareBuilder.utils.hooks.copy_metadata`.
  (:issue:`5830`)
...
* Fix python library lookup when building with RH SCL python 3.8 or later.
  (:issue:`5749`)
* Prevent :func:`MwlareBuilder.utils.hooks.copy_metadata` from renaming
...
* * Add ``PostGraphAPI.analysis`` attribute.
  ...
  * Hooks may access a ``Analysis.hooksconfig`` attribute
    ...
    A helper function :func:`~MwlareBuilder.utils.hooks.get_hook_config`
...
* Add Tcl/Tk based Splash screen, which is controlled from
  ...
  screen in MwlareBuilder is under :mod:`Splash` available.
  A huge thanks to `@Chrisg2000 <https://github.com/Chrisg2000>`_ for
...
* Add new hook utility :func:`~MwlareBuilder.utils.hooks.collect_entry_point` for
...
* (OSX) Suppress missing library error messages for system libraries as
  those are never collected by MwlareBuilder and starting with Big Sur,
...
* (Windows) Change default cache directory to ``LOCALAPPDATA``
  ...
  For this and future versions ``AppData\Roaming\mwlarebuilder``
...
* MwlareBuilder-frozen onefile programs are now compatible with ``staticx``
...
* (OSX) Automatically remove the signature from the collected copy of the
  ...
  versions for macOS, where invalidated signature on MwlareBuilder-collected
  copy of the ``Python`` library prevents the latter from being loaded.
  (:issue:`5451`)
* (Windows) MwlareBuilder's console or windowed icon is now added at freeze-time
  and
...
* (Windows) MwlareBuilder's default icon is no longer built into the bootloader,
  but
...
* Fix AppImage builds that were broken since MwlareBuilder 3.6. (:issue:`4693`)
...
* Provide setuptools entrypoints to enable other packages to provide
  MwlareBuilder hooks specific to that package, along with tests for these
  hooks.

  Maintainers of Python packages requiring hooks are invited to use this new
  feature and provide up-to-date MwlareBuilder support along with their package.
  ...
  __ https://github.com/mwlarebuilder/hooksample

* A new package `mwlarebuilder-hooks-contrib`__ provides monthly updated hooks
  now. This package is installed automatically when installing MwlareBuilder,
  but can be updated independently.
  Many thanks to Legorooj for setting up the new package
  and moving the hooks there.

  __ https://github.com/mwlarebuilder/mwlarebuilder-hooks-contrib
...
* Change the hook collection order so that the hook-priority is command line,
  then entry-point, then MwlareBuilder builtins. (:issue:`4876`)
...
* Remove support for Python 2.7. The minimum required version is now Python
  3.5. The last version supporting Python 2.7 was MwlareBuilder 3.6.
  (:issue:`4623`)
* Many hooks are now part of the new `mwlarebuilder-hooks-contrib`
...
* These hooks have been moved to the new
  `mwlarebuilder-hooks-contrib`__ repository:
...
* These hooks have been added while now moved to the new
  `mwlarebuilder-hooks-contrib` repository: astor (:issue:`4400`,
...
* These changes have been applied to hooks now moved to the new
  `mwlarebuilder-hooks-contrib` repository
...
* Provide setuptools entrypoints to enable other packages to provide
  MwlareBuilder hooks specific to that package, along with tests for these hooks.
  See https://github.com/mwlarebuilder/hooksample for more information.
