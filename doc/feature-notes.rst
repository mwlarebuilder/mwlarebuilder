...
caused <1.4 MwlareBuilder import detect machinery to miss those libraries,
failing the goal to build self-contained MwlareBuilder executables::

  from ctypes import *
  # This will pass undetected under MwlareBuilder detect machinery,
  # because it's not a direct import.
  handle = CDLL("/usr/lib/library.so")
  handle.function_call()


Solution in MwlareBuilder
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

MwlareBuilder contains a pragmatic implementation of Ctypes dependencies: it
...
detected::
  ...
  cdll.library # Only valid under Windows - a limitation of ctypes, not MwlareBuilder's
  windll.library # Only valid under Windows - a limitation of ctypes, not MwlareBuilder's
...
* **only library paths represented by a literal string will be detected and
  included in the final executable**: MwlareBuilder import detection works by
...
If MwlareBuilder does not detect a library, you can add it to your
...
MwlareBuilder tries to detect binary modules created by SWIG. This detection
requires:
...
MwlareBuilder can follow import statements that refer to Cython C object
modules and bundle them – like for any other module implemented in C.

But – again, as for any other module implemented in C – MwlareBuilder can not
...
MwlareBuilder aims to support all possible combinations stemming from
the above options:
...
**By default, MwlareBuilder targets the current running architecture
...
.. versionchanged:: 4.10
   In earlier MwlareBuilder versions, the architecture validation was performed
   on all collected binaries, such as python extension modules and the
   shared libraries referenced by those extensions. As of MwlareBuilder 4.10,
...
The processing of binaries done by MwlareBuilder (e.g., library path
...
**By default, MwlareBuilder ad-hoc (re)signs all collected binaries and
...
.. note::
   When codesign identity is specified, MwlareBuilder also turns on
...
MwlareBuilder also automatically attempts to sign `.app bundles`, either
...
MwlareBuilder provides two aspects of support for macOS event handling;
...
.. versionchanged:: 5.0
   In earlier MwlareBuilder versions, `argv emulation` was always enabled
   in ``onefile`` mode and was unavailable in ``onedir`` mode.
   As MwlareBuilder 5.0, `argv emulation` must be explicitly opted-in,
...
In MwlareBuilder ``onedir`` bundles, the application runs as a single
...
MwlareBuilder implements an optional feature called `argv emulation`,
...
.. warning::
   ...
   While MwlareBuilder tries to mitigate the issue on its end, we recommend
...
MwlareBuilder automatically generates an ``Info.plist`` file for your
...
Enabling `argv emulation` in MwlareBuilder causes its bootloader to
...
In MwlareBuilder-frozen ``onefile`` bundles, the child process always
...
python script, and also when frozen by MwlareBuilder as a
:option:`onedir <--onedir>` application. However, :option:`onefile <--onefile>`
applications frozen with MwlareBuilder versions prior to 5.3 exhibit a
...
The :option:`onefile <--onefile>` mode in MwlareBuilder uses two processes.
...
in MwlareBuilder 5.3 and later attempts to delay the shut-down of the
...
This behavior is readily available in any MwlareBuilder version; in
...
This behavior was implemented in MwlareBuilder 5.3 to ensure that closing
...
* build type: :option:`onedir <--onedir>` (single-process) vs.
  :option:`onefile <--onefile>` (two-process) MwlareBuilder build option.

* console enabled or not: :option:`console <--console>` vs.
  :option:`noconsole/windowed <--noconsole>` MwlareBuilder build option.
...
really affect the clean-up from MwlareBuilder's perspective. But it may be
...
For console-enabled Windows applications, MwlareBuilder offers an option
...
The advantage of having it in MwlareBuilder's bootloader is that:
...
console-enabled builds, and involves MwlareBuilder's bootloader
