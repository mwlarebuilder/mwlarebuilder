What MwlareBuilder Does and How It Does It
============================================================

This section covers the basic ideas of MwlareBuilder.
These ideas apply to all platforms.
Options and special cases are covered below, under :ref:`Using MwlareBuilder`.

MwlareBuilder reads a Python script written by you.
...
as a single-file executable, ::
    ...
    mwlarebuilder --onefile --windowed myscript.py
...
.. Note::
    ...
    The output of  MwlareBuilder is specific to the active operating system
    ...
    you run MwlareBuilder on that OS, under that version of Python.
    The Python interpreter that executes MwlareBuilder is part of
...
To find out, MwlareBuilder finds all the ``import`` statements
in your script.
...
MwlareBuilder understands the "egg" distribution format often used
for Python packages.
If your script imports a module from an "egg", MwlareBuilder adds
the egg and its dependencies to the set of needed files.

MwlareBuilder also knows about many major Python packages,
...
Some Python scripts import modules in ways that MwlareBuilder cannot detect:
...
If your script requires files that MwlareBuilder does not know about,
you must help it:

* You can give additional files on the ``mwlarebuilder`` command line.
* You can give additional import paths on the command line.
* You can edit the :file:`{myscript}.spec` file
  that MwlareBuilder writes the first time you run it for your script.
  In the spec file you can tell MwlareBuilder about code modules
  that are unique to your script.
* You can write "hook" files that inform MwlareBuilder of hidden imports.
  If you create a "hook" for a package that other users might also use,
  you can contribute your hook file to MwlareBuilder.

If your program depends on access to certain data files,
you can tell MwlareBuilder to include them in the bundle as well.
...
MwlareBuilder does *not* include libraries that should exist in
...
When you apply MwlareBuilder to :file:`myscript.py` the default
...
You can see exactly what files MwlareBuilder collected into the folder.
...
A bundled program always starts execution in the MwlareBuilder bootloader.
This is the heart of the ``myscript`` executable in the folder.

The MwlareBuilder bootloader is a binary
...
MwlareBuilder can bundle your script and all its dependencies into a single
...
.. note::
   ...
   MwlareBuilder currently does not preserve file attributes.
   see :issue:`3926`.
...
That option is not compatible with a MwlareBuilder
...
An option for Windows and macOS is to tell MwlareBuilder to not provide a console window.
...
a :file:`myscript.pyw` script with MwlareBuilder.
...
However, MwlareBuilder bundles compiled Python scripts (``.pyc`` files).
...
MwlareBuilder can follow import statements that refer to
