...
The syntax of the ``mwlarebuilder`` command is:

    ``mwlarebuilder`` [*options*] *script* [*script* ...] | *specfile*

In the most simple case,
set the current directory to the location of your program ``myscript.py``
and execute::

    mwlarebuilder myscript.py

MwlareBuilder analyzes :file:`myscript.py` and:
...
You may give a path to the script or spec file, for example
    ...
    ``mwlarebuilder`` `options...` ``~/myproject/source/myscript.py``

or, on Windows,

    ``mwlarebuilder "C:\Documents and Settings\project\myscript.spec"``


Options
~~~~~~~~~~~~~~~

A full list of the ``mwlarebuilder`` command's options are as follows:

.. include:: _mwlarebuilder-options.tmp


Shortening the Command
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Because of its numerous options, a full ``mwlarebuilder`` command
...
For example, in GNU/Linux::
    ...
    mwlarebuilder --noconfirm --log-level=WARN \
        --onefile --nowindow \
        --add-data="README:." \
...
Running MwlareBuilder from Python code
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If you want to run MwlareBuilder from Python code, you can use the ``run`` function
defined in ``MwlareBuilder.__main__``. For instance, the following code:

.. code-block:: python

    import MwlareBuilder.__main__

    MwlareBuilder.__main__.run([
        'my_script.py',
        '--onefile',
        '--windowed'
    ])
...
.. code-block:: shell
    ...
    mwlarebuilder my_script.py --onefile --windowed
...
When UPX is available, MwlareBuilder uses it to individually compress
...
MwlareBuilder looks for the UPX in the standard executable path(s) (defined
...
.. versionchanged:: 4.2
    MwlareBuilder detects CFG-enabled DLLs and automatically excludes
    them from UPX processing.

.. versionchanged:: 4.3
    MwlareBuilder automatically excludes Qt5 and Qt6 plugins from
    UPX processing.

Although MwlareBuilder attempts to automatically detect and exclude some of
...
have been `reported <https://github.com/mwlarebuilder/mwlarebuilder/issues/4178#issuecomment-868985789>`_
...
module `tkinter`_. MwlareBuilder bundles the dynamic libraries of tcl and tk into the
...
because it is part of MwlareBuilder and is included as needed.
This module must be imported within the Python program. The usage is as follows::

    import pyi_splash

    # Update the text on the splash screen
    pyi_splash.update_text("MwlareBuilder is a great software!")
...
just install MwlareBuilder like any other package and use it in your
...
* Install MwlareBuilder in each virtual environment.
* Use MwlareBuilder to build your application in each virtual environment.

Note that when using `venv`, the path to the MwlareBuilder commands is:
...
for example both Windows and macOS, you must install MwlareBuilder
...
In it you install
Python, the support packages your application needs, and MwlareBuilder.
...
MwlareBuilder reads scripts from the common synchronized folder,
...
example GNU/Linux and macOS, you will need to set the MWLAREBUILDER_CONFIG_DIR
environment variable to different values on each platform otherwise
MwlareBuilder may cache files for one platform and use them on the other
...
For this reason MwlareBuilder includes the ``pyi-grab_version`` command.
...
option to ``mwlarebuilder`` or ``pyi-makespec``.
...
``utils/win32/versioninfo.py`` in the MwlareBuilder distribution folder.
...
Under macOS, MwlareBuilder always builds a UNIX executable in
:file:`dist`.
...
The one built by MwlareBuilder contains a folder always named
...
(If you do not specify an icon file, MwlareBuilder supplies a
file :file:`icon-windowed.icns` with the MwlareBuilder logo.)
...
(see the `MwlareBuilder code signing recipe`_
...
Under GNU/Linux, MwlareBuilder does not bundle ``libc``
...
Note that MwlareBuilder does bundle other shared libraries that are discovered
...
3. Install the `Windows Software Development Kit (SDK) for Windows 10
   ...
   If you think, MwlareBuilder should do this by itself, please :ref:`help
   improving <how-to-contribute>` MwlareBuilder.
...
MwlareBuilder does not collect system components of the OS, the collected
...
you should set up a full environment (i.e., install python, MwlareBuilder,
your application's code, and all its dependencies) in a copy of macOS 10.14,
using a virtual machine if necessary. Then use MwlareBuilder to freeze
...
.. note:: This section is largely obsolete, as support for 32-bit application
          ...
          However, MwlareBuilder still supports building 32-bit bootloader,
          and 32-bit/64-bit Python installers are still available from
          python.org for (some) versions of Python 3.7 which MwlareBuilder dropped
          support for in v6.0.

Older versions of macOS supported both 32-bit and 64-bit executables.
MwlareBuilder builds an app using the the word-length of the Python used to execute it.
That will typically be a 64-bit version of Python,
resulting in a 64-bit executable.
To create a 32-bit executable, run MwlareBuilder under a 32-bit Python.
...
.. note:: MwlareBuilder does not provide pre-built 32-bit bootloaders for
          macOS anymore. In order to use MwlareBuilder with 32-bit python,
...
Alternatively, MwlareBuilder also supports conversion of open
