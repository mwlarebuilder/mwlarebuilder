...
MwlareBuilder comes with pre-compiled bootloaders for some platforms in
...
* :file:`../MwlareBuilder/bootloader/{OS_ARCH}/run`,
* :file:`../MwlareBuilder/bootloader/{OS_ARCH}/run_d`,
* :file:`../MwlareBuilder/bootloader/{OS_ARCH}/runw` (macOS and Windows only), and
* :file:`../MwlareBuilder/bootloader/{OS_ARCH}/runw_d` (macOS and Windows only).
...
By setting the environment variable ``MWLAREBUILDER_COMPILE_BOOTLOADER``
the pip_ setup will attempt to build the bootloader for your platform, even
if it is already present. Doing so would execute the command ``python ./waf configure all`` upon installation. You can also pass additional arguments to the build process by setting the ``MWLAREBUILDER_BOOTLOADER_WAF_ARGS`` environment variable.
...
Docker_.
The `Dockerfile <https://github.com/mwlarebuilder/mwlarebuilder/tree/develop/bootloader/Dockerfile>`_ contains the instructions on how to do this.
...
* :file:`../MwlareBuilder/bootloader/Darwin-{*}/`.
...
The pre-compiled bootloader coming with MwlareBuilder are
...
1. Using the Visual Studio C++ compiler.
   ...
   This is why the bootloaders delivered with MwlareBuilder are build using
...
.. note:: The correct setting of :envvar:`OBJECT_MODE` is also needed when you
   use MwlareBuilder to package your application.
...
MwlareBuilder maintains a set of virtual machine description for testing and
...
the MwlareBuilder distribution folder and will put the built executables onto
the build-host (into :file:`../MwlareBuilder/bootloader/`).
...
Example usage::
  ...
  git status ../MwlareBuilder/bootloader/
