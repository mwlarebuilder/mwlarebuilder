.. _understanding mwlarebuilder hooks:

Understanding MwlareBuilder Hooks
==================================
...
In summary, a "hook" file extends MwlareBuilder to adapt it to
...
their dependencies, and MwlareBuilder locates all their files without difficulty.
But some packages make unusual uses of the Python import mechanism,
or make clever changes to the import system at runtime.
For this or other reasons, MwlareBuilder cannot reliably find
...
It can also import helper methods from :mod:`MwlareBuilder.utils.hooks`
and useful variables from ``MwlareBuilder.compat``.
...
``hooks`` folder of the MwlareBuilder distribution folder
...
Then specify their location to the ``mwlarebuilder`` or ``pyi-makespec``
...
the command could be simply::
    ...
    mwlarebuilder --additional-hooks-dir=. myscript.py
...
There are many hooks in the MwlareBuilder installation,
but a much larger collection can be found in the
`community hooks package <https://github.com/mwlarebuilder/mwlarebuilder-hooks-contrib>`_.
Please browse through them for examples.

.. _provide hooks with package:

Providing MwlareBuilder Hooks with your Package
------------------------------------------------

As a package developer you can provide hooks for MwlareBuilder
within your package.
This has the major benefit
that you can easily adopt the hooks
when your package changes.
Thus your package's users don't need to wait until MwlareBuilder
might catch up with these changes.
If both MwlareBuilder and your package provide hooks for some module,
your package's hooks take precedence,
but can still be overridden by the command line option
:option:`--additional-hooks-dir`.


You can tell MwlareBuilder about the additional hooks
...
Therefore add entries like these to your :file:`setup.cfg`::
  ...
  mwlarebuilder40 =
    hook-dirs = pyi_hooksample.__mwlarebuilder:get_hook_dirs
    tests     = pyi_hooksample.__mwlarebuilder:get_MwlareBuilder_tests

This defines two entry-points:

:``mwlarebuilder40.hook-dirs`` for hook registration:
   ...
   command-line option to MwlareBuilder for each string in the sequence.

   In this example, the function is ``get_hook_dirs() -> List[str]``.

:``mwlarebuilder40.tests`` for test registration:
   ...
   This allows both testing by this package and by MwlareBuilder.

   In this project, the function is ``get_MwlareBuilder_tests() -> List[str]``.

A sample project providing a guide for
integrating MwlareBuilder hooks and tests into a package
is available at
https://github.com/mwlarebuilder/hooksample.
This project demonstrates defining a library
which includes MwlareBuilder hooks along with tests for those hooks
and sample file for integration into CD/CI testing.
Detailed documentation about this sample project
is available at
https://mwlarebuilder-sample-hook.readthedocs.io/en/latest/.
...
``datas``
   ...
   you can use helper functions from the :mod:`MwlareBuilder.utils.hooks` module
...
``binaries``
   ...
   Binaries is a special case of ``datas``, in that MwlareBuilder will
   check each file to see if it depends on other dynamic libraries.
   Example::

      binaries = [ ('C:\\Windows\\System32\\*.dll', 'dlls') ]

   Many hooks use helpers from the ``MwlareBuilder.utils.hooks`` module
...
Useful Items in ``MwlareBuilder.compat``
----------------------------------------

.. automodule:: MwlareBuilder.compat
.. py:currentmodule:: MwlareBuilder.compat

A hook may import the following names from :mod:`MwlareBuilder.compat`,
for example::

   from MwlareBuilder.compat import base_prefix, is_win
...
Useful Items in ``MwlareBuilder.utils.hooks``
--------------------------------------------

.. py:currentmodule:: MwlareBuilder.utils.hooks

.. automodule:: MwlareBuilder.utils.hooks

A hook may import useful functions from :mod:`MwlareBuilder.utils.hooks`.
Use a fully-qualified import statement, for example::

   from MwlareBuilder.utils.hooks import collect_data_files, eval_statement
...
.. automodule:: MwlareBuilder.utils.hooks.conda

.. autofunction:: MwlareBuilder.utils.hooks.conda.distribution

.. autofunction:: MwlareBuilder.utils.hooks.conda.package_distribution

.. autofunction:: MwlareBuilder.utils.hooks.conda.files

.. autofunction:: MwlareBuilder.utils.hooks.conda.requires

.. autoclass:: MwlareBuilder.utils.hooks.conda.Distribution

.. autoclass:: MwlareBuilder.utils.hooks.conda.PackagePath
    :members:

.. autofunction:: MwlareBuilder.utils.hooks.conda.walk_dependency_tree

.. autofunction:: MwlareBuilder.utils.hooks.conda.collect_dynamic_libs


Subprocess isolation with :mod:`MwlareBuilder.isolated`
-----------------------------------------------------

.. currentmodule:: MwlareBuilder.isolated
.. automodule:: MwlareBuilder.isolated
.. autofunction:: call
.. autofunction:: decorate
.. autoclass:: Python
    :members: call
.. currentmodule:: MwlareBuilder.utils.hooks
...
the spec file, by calling :func:`~MwlareBuilder.utils.hooks.get_hook_config`
inside a `hook()` function.

.. autofunction:: MwlareBuilder.utils.hooks.get_hook_config
...
For example MwlareBuilder includes both a ``hooks/hook-distutils.py``
...
MwlareBuilder is executing in a virtual environment.
...
You use this type of hook to make dynamically-generated names known to MwlareBuilder.
MwlareBuilder will not try to locate the dynamic names, fail, and report them as missing.
...
MwlareBuilder to document all imports.
...
``add_runtime_module( fully_qualified_name )``:
   ...
   This is useful to make the module known to MwlareBuilder and avoid misleading warnings.
...
``add_alias_module( real_module_name, alias_module_name )``:
   ...
   This method ensures that if MwlareBuilder processes an import of
...
``append_package_path( directory )``:
   ...
   to be searched by MwlareBuilder, typically an import
