...
check "are we bundled?"::
    ...
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        print('running in a MwlareBuilder bundle')
    else:
...
from a bundled script, the MwlareBuilder bootloader will set the module's
...
it is always an absolute path. In the bundled script, the MwlareBuilder
...
.. versionchanged:: 4.3
    ...
    Therefore, MwlareBuilder documentation used to suggest ``sys._MEIPASS``
...
meaning that if you put ``file.dat`` in the root of your package, using::
    ...
    MwlareBuilder --add-data="/path/to/file.dat:."
...
``[app root]/my_library/data.pyc`` and :option:`--add-data` should mirror that::
    ...
    MwlareBuilder --add-data="/path/to/my_library/file.dat:./my_library"

However, in this case it is much easier to switch to :ref:`the spec file
<Using Spec Files>` and use the
:func:`MwlareBuilder.utils.hooks.collect_data_files` helper function::
...
If it exists,
MwlareBuilder saves the original value to `*_ORIG`, then modifies the search
