...
MwlareBuilder runs in Windows 8 and newer. It can create graphical windowed apps
(apps that do not need a command window).

macOS
~~~~~~

MwlareBuilder runs on macOS 10.15 (Catalina) or newer.
It can build graphical windowed apps (apps that do not use a terminal window).
MwlareBuilder builds apps that are compatible with the macOS release in
...
MwlareBuilder requires the ``ldd`` terminal application to discover
...
Users have reported success running MwlareBuilder on these platforms,
...
When you install MwlareBuilder using pip_, the setup will attempt
to build a bootloader for this platform.
If that succeeds, the installation continues and MwlareBuilder is ready to use.
