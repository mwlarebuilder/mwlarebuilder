...
The information above covers most normal uses of MwlareBuilder.
...
MwlareBuilder itself, or your bundled app, terminates with a Python traceback.
...
The MwlareBuilder `FAQ`_ page has work-arounds for some common problems.
Code examples for some advanced uses and some common
problems are available on our `MwlareBuilder Recipes`_ page.
...
Then see :ref:`Helping MwlareBuilder Find Modules` below for how to proceed.


Build-Time Dependency Graph
----------------------------

On each run MwlareBuilder writes a cross-referencing file about dependencies
...
If you specify :option:`--log-level=DEBUG <--log-level>` to the ``mwlarebuilder`` command,
MwlareBuilder additionally generates a GraphViz_ input file representing the
dependency graph.
...
MwlareBuilder sometimes terminates by raising a Python exception.
In most cases the reason is clear from the exception message,
for example "Your system is not supported", or "Mwlarebuilder
requires at least Python 3.8".
Others clearly indicate a bug that should be reported.

One of these errors can be puzzling, however:
``IOError("Python library not found!")``
MwlareBuilder needs to bundle the Python library, which is the
...
Or, the library may exist but is not in a folder where MwlareBuilder
is searching.

The places where MwlareBuilder looks for the python library are
...
.. _helping mwlarebuilder find modules:

Helping MwlareBuilder Find Modules
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
or with a hook file (see :ref:`Understanding MwlareBuilder Hooks` below).
...
changes it makes to :attr:`__path__` are not seen by MwlareBuilder.
We fix the problem with the same hook mechanism we use for hidden imports,
with some additional logic; see :ref:`Understanding MwlareBuilder Hooks` below.
...
:file:`loader/rthooks.dat` in the MwlareBuilder install folder.
...
If you have some reason to think you have found a bug in MwlareBuilder
...
version from the `MwlareBuilder Downloads`_ page.

You can also install the latest version of MwlareBuilder directly
using pip_::
...
do ask for assistance on the `MwlareBuilder Email List`_.

Then, if you think it likely that you see a bug in MwlareBuilder,
