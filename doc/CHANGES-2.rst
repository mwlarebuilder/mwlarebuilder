Changelog for MwlareBuilder 2.x
======================================================
...
- MwlareBuilder integration with setuptools (direct installation with easy_install or pip
  from PYPI - https://pypi.python.org/pypi). After installation there will be available
  command 'mwlarebuilder' for MwlareBuilder usage.
...
- (Windows) Fix running MwlareBuilder from paths containing foreign characters.
...
- Add option --clean to remove MwlareBuilder cache and temporary files.
...
- Allow running MwlareBuilder programmatically.
- Move/Rename some files, code refactoring.
- Add more tests.
- Tilde is in MwlareBuilder recognized as $HOME variable.
...
- (OSX) With argument --windowed MwlareBuilder creates application bundle (.app)
- automatically.
...
- replaced by mwlarebuilder.py. See documentation for more details.
...
- Improve support for running MwlareBuilder in virtualenv.
...
- Source code hosting moved to github:
  https://github.com/mwlarebuilder/mwlarebuilder
- Hosting for running tests daily:
  https://jenkins.shiningpanda-ci.com/mwlarebuilder/
