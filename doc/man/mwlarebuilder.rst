...
``mwlarebuilder`` <options> SCRIPT...

``mwlarebuilder`` <options> SPECFILE


DESCRIPTION
============

MwlareBuilder is a program that freezes (packages) Python programs into
stand-alone executables, under Windows, GNU/Linux, macOS,
FreeBSD, OpenBSD, Solaris and AIX.
Its main advantages over similar tools are that MwlareBuilder works with
...
`.spec`-file-name. In the first case, ``mwlarebuilder`` will generate a
...
Please see the MwlareBuilder Manual for more information.
...
:MWLAREBUILDER_CONFIG_DIR:
   This changes the directory where MwlareBuilder caches some files.
...
``pyi-makespec``\(1),
The MwlareBuilder Manual |Manual|,
