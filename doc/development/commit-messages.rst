...
MwlareBuilder is maintained since 2005 and we often need to
...
* ``depend`` for the dependency detection parts (:file:`MwlareBuilder/depend`)

* ``building`` for the building part (:file:`MwlareBuilder/building`)

* ``compat`` for code related to compatibility of different Python versions
  (primary :file:`MwlareBuilder/compat.py`)
...
* ``modulegraph``: changes related to :file:`MwlareBuilder/lib/modulegraph`
...
working on on this system. To set it for just the MwlareBuilder repo, remove the
``--global`` flag.
