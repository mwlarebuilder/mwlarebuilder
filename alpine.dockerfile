# Setup an Alpine test environment with Python, MwlareBuilder and its test dependencies.
#
...
#
# ---
# This dockerfile is 2-part. The first half builds and the second tests. The test half should only contain MwlareBuilder's
...
# Build a wheel for MwlareBuilder. Do this last and use as few files as possible to maximize cache-ability.
COPY COPYING.txt .
COPY setup.* ./
COPY bootloader bootloader
COPY MwlareBuilder MwlareBuilder
