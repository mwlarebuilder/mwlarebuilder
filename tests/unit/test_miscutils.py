...
from MwlareBuilder import HOMEPATH, PLATFORM
from MwlareBuilder.utils.misc import load_py_data_struct, save_py_data_struct


@pytest.mark.win32
def test_versioninfo(tmp_path):
    from MwlareBuilder.utils.win32.versioninfo import VSVersionInfo, \
...
def test_versioninfo_written_to_exe(tmp_path):
    ...
    bootloader_file = os.path.join(HOMEPATH, 'MwlareBuilder', 'bootloader', PLATFORM, 'run.exe')
...
def test_decode():
    """
    Test MwlareBuilder.utils.misc.decode().
    """
    import codecs
    from MwlareBuilder.utils.misc import decode
