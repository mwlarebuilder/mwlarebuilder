...
from MwlareBuilder import HOMEPATH
from MwlareBuilder.depend import analysis
from MwlareBuilder.lib.modulegraph import modulegraph
import MwlareBuilder.log as logging
from MwlareBuilder.utils.tests import gen_sourcefile
...
def test_metadata_collection(tmpdir):
    from MwlareBuilder.utils.hooks import copy_metadata
...
def test_metadata_searching():
    ...
    # This test analyses code which implies that MwlareBuilder's own metadata (and possibly that of its dependencies) is
    # required.
    mwlarebuilder = set(copy_metadata("mwlarebuilder"))
    with_dependencies = set(copy_metadata("mwlarebuilder", recursive=True))

    self = FakeGraph("from importlib.metadata import distribution; distribution('mwlarebuilder')")
    assert mwlarebuilder == self.metadata_required()

    self = FakeGraph("import pkg_resources; pkg_resources.get_distribution('mwlarebuilder')")
    assert mwlarebuilder == self.metadata_required()

    self = FakeGraph("import pkg_resources; pkg_resources.require('mwlarebuilder')")
