...
from MwlareBuilder.utils.hooks import collect_data_files, collect_submodules, \
    get_module_file_attribute, remove_prefix, remove_suffix, \
    remove_file_extension, is_module_or_submodule, \
    check_requirement
from MwlareBuilder.compat import exec_python, is_win
from MwlareBuilder import log as logging
...
class TestCollectSubmodules(object):
    ...
    def test_collect_submod_module(self, caplog):
        with caplog.at_level(logging.DEBUG, logger='MwlareBuilder.utils.hooks'):
    ...
    def test_collect_submod_egg(self, tmpdir, monkeypatch):
        ...
        monkeypatch.setattr('MwlareBuilder.config.CONF', {'pathex': [pth]})
    ...
    def test_collect_submod_stdout_interference(self, monkeypatch):
        ...
        monkeypatch.setattr('MwlareBuilder.config.CONF', {'pathex': [TEST_MOD_PATH]})
        monkeypatch.syspath_prepend(TEST_MOD_PATH)
