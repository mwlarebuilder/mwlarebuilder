#-----------------------------------------------------------------------------
# Copyright (c) 2022-2023, MwlareBuilder Development Team.
#
...
from MwlareBuilder.utils.tests import importorskip
from MwlareBuilder.building.osx import DOT_REPLACEMENT

# NOTE: the tests below explicitly enable the following environment variables:
#  MWLAREBUILDER_STRICT_BUNDLE_CODESIGN_ERROR: have codesign errors terminate the build instead of generating a warning.
#  MWLAREBUILDER_VERIFY_BUNDLE_SIGNATURE: perform strict verification of signature and signature-related data.
...
def test_macos_bundle_signing_metadata(pyi_builder, monkeypatch):
    monkeypatch.setenv("MWLAREBUILDER_STRICT_BUNDLE_CODESIGN_ERROR", "1")
    monkeypatch.setenv("MWLAREBUILDER_VERIFY_BUNDLE_SIGNATURE", "1")
...
def test_macos_bundle_signing_py_files(pyi_builder, monkeypatch):
    ...
    Analysis = MwlareBuilder.building.build_main.Analysis
    monkeypatch.setattr('MwlareBuilder.building.build_main.Analysis', AnalysisOverride)
...
def _create_test_framework(bundle_path):
    ...
    info_plist = {
        ...
        'CFBundleIdentifier': f'org.mwlarebuilder.{binary_name}',
