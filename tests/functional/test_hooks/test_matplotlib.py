...
def test_matplotlib(pyi_builder, monkeypatch, backend_name, package_name, binding):
    ...
    # MwlareBuilder options excluding all backend packages except the passed backend package. This is especially critical
    # for Qt backend packages (e.g., "PyQt5", "PySide2"). On first importation, Matplotlib attempts to import all
    # available Qt packages. However, runtime MwlareBuilder hooks fail when multiple Qt packages are frozen into the same
