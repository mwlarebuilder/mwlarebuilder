...
import MwlareBuilder.depend.utils
from MwlareBuilder.utils.tests import skipif, importorskip, skipif_no_compiler, xfail, has_compiler
...
def test_nameclash(pyi_builder):
    # test-case for issue #964: Nameclashes in module information gathering All mwlarebuilder specific module attributes
...
def test_error_during_import(pyi_builder):
    # See ticket #27: historically, MwlareBuilder was catching all errors during imports...
    pyi_builder.test_source(
        """
        try:
...
__orig_resolveCtypesImports = MwlareBuilder.depend.utils._resolveCtypesImports


def __monkeypatch_resolveCtypesImports(monkeypatch, compiled_dylib):
    def mocked_resolveCtypesImports(*args, **kwargs):
        from MwlareBuilder.config import CONF
    ...
    monkeypatch.setattr(MwlareBuilder.depend.utils, "_resolveCtypesImports", mocked_resolveCtypesImports)


#FIXME: For reusability, move this to "MwlareBuilder.utils.tests".
...
def test_ctypes_gen(pyi_builder, monkeypatch, funcname, compiled_dylib, test_id):
    ...
    # only, not its path. See discussion in https://github.com/mwlarebuilder/mwlarebuilder/pull/1478#issuecomment-139622994.
...
def test_egg_unzipped(pyi_builder):
    ...
    pyi_builder.test_source(
        """
        # This code is part of the package for testing eggs in `MwlareBuilder`.
        import os
...
def test_egg_unzipped_metadata_pkg_resources(pyi_builder):
    ...
    pyi_builder.test_source(
        ...
        # (which is picked up by MwlareBuilder's bytecode analysis).
