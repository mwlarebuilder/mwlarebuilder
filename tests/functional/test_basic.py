...
from MwlareBuilder.compat import is_darwin, is_win
from MwlareBuilder.utils.tests import importorskip, skipif, skipif_no_compiler, xfail
...
def test_pyz_as_external_file(pyi_builder, monkeypatch):
    ...
    import MwlareBuilder.building.build_main
    EXE = MwlareBuilder.building.build_main.EXE
    monkeypatch.setattr('MwlareBuilder.building.build_main.EXE', MyEXE)
...
def test_ctypes_hooks_are_in_place(pyi_builder):
    pyi_builder.test_source(
        """
        import ctypes
        assert ctypes.CDLL.__name__ == 'MwlareBuilderCDLL', ctypes.CDLL
        """
    )
...
def test_load_dll_using_ctypes(monkeypatch, pyi_builder, compiled_dylib):
    ...
    #
    # TODO: make sure MwlareBuilder is able to find the library and bundle it with the app.
...
def test_option_exclude_module(pyi_builder):
    ...
    pyi_builder.test_source(
        ...
        try:
            ...
            # The Import error is expected since MwlareBuilder should
...
def test_hook_collect_submodules(pyi_builder, script_dir):
    # This is designed to test the operation of MwlareBuilder.utils.hook.collect_submodules. To do so:
    #
...
# Test that MwlareBuilder can handle a script with an arbitrary extension.
...
def test_application_executable_has_manifest(pyi_builder, icon):
    ...
    """
    from MwlareBuilder.utils.win32 import winmanifest
    from MwlareBuilder import PACKAGEPATH
    ...
    if icon == 'icon_default':
        ...
        # Locate mwlarebuilder's default icon, and explicitly give it
        # via --icon argument
...
def test_subprocess_in_windowed_mode(pyi_windowed_builder):
    """Test invoking subprocesses from a MwlareBuilder app built in windowed mode."""
...
def test_package_entry_point_name_collision(pyi_builder):
    ...
    still handles this case fine and MwlareBuilder should too.
    """
