...
open https://readthedocs.org/projects/mwlarebuilder/builds/
...
set -x MWLAREBUILDER_DO_RELEASE 1
pip install -q -r doc/requirements.txt
pip install -Uq twine

read -n1 -P 'nano is about to be opened so that you can edit MwlareBuilder\'s version. When it does, write the new version then save and quit. Press any key to proceed: '
nano +21 MwlareBuilder/__init__.py
towncrier --yes
...
set title 'Contributions to MwlareBuilder '(python setup.py --version)
...
perl -pe 's&https?://mwlarebuilder.readthedocs.io&https://mwlarebuilder.org&g' -i README.rst
perl -pe 's&(https://mwlarebuilder.org/en/)[^/]+/&$1'v(python setup.py --version)'/&g' -i README.rst
...
read -P 'If the bootloaders need to be rebuilt, now is the time to do it. Hit return if they don\'t need rebuilding or once you have rebuilt and copied the '(set_color --bold)'Windows and macOS bootloaders only'(set_color normal)' into MwlareBuilder/bootloaders: '
...
function pyi_build_wheels
    rm -rf build dist mwlarebuilder.egg-info
...
function pyi_github_release
    gh release create v(python setup.py --version) --notes 'Please see the [v'(python setup.py --version)' section of the changelog](https://mwlarebuilder.org/en/v'(python setup.py --version)'/CHANGES.html#id1) for a list of the changes since '(git tag -l --sort=version:refname)[-2]'.'
end
